 ...
 * @param {Function} callback An optional callback. If provided, it should handle the stream ending.
 */
Wrabbit.prototype.stream = function (url, wrapline, stream, callback) {
    var self = this;
...
Wrabbit.prototype.server = function (options, callback) {
    ...
    server.page.add(/^\/api\/https?\/.*/, function (lien) {
        ...
        self.stream(url, lien.search.w === "1" ? "E('http://" + lien.req.headers.host + lien.req.url + "', function (require, module, exports, global, engine) {" : "", lien.res, function (err) {
